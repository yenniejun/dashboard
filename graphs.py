...
def noise_graph(graph_type):
	...
	elif graph_type=="borough":
		fig = px.line(grouper_bor, 
                 x=[a[0] for a in grouper_bor.index], 
                 y=grouper_bor.unique_key, 
                 color=[a[1] for a in grouper_bor.index],
                 title='DAILY NYC Noise Complaints (By Borough)',
                 labels = {'x':'Date', 'unique_key':'Number of Noise Complaints'})
	elif graph_type=="zip":
		fig = px.line(grouper_zip, 
                 x=[a[0] for a in grouper_zip.index], 
                 y=grouper_zip.unique_key, 
                 color=[a[1] for a in grouper_zip.index],
